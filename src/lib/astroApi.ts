...
import type { PostHistoryType, PostType } from '../types/post'
...
const fetchHistory = async (fullPath: string): Promise<PostHistoryType> => {
  try {
    ...
    const format = `${commitSeparator}%cd,%H,%s`

    const { stdout } = await safeExec(
      'git',
      ['log', `--format=${format}`, '--date=iso8601-strict', '--', safePath],
      {
        cwd: process.cwd(),
        timeout: 10000,
      },
    ...
    return stdout
      ...
      .map((commit) => {
        ...
        return { date, hash, message: messageParts.join(',') }
      })
...
const postEntryToPost = async (entry: PostEntry, fields: string[] = []): Promise<PostType> => {
  ...
  return {
    ...
    ...(fields.includes('history') && { history: await fetchHistory(basePath) }),
